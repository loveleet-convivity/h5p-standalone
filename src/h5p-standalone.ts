...
interface Options {
  ...
  ajax?: {
    setFinishedUrl?: () => Promise<any>;
    contentUserDataUrl?: () => Promise<any>;
  };
...
export class H5PStandalone {
  ...
  options: Options; // Add the options property
  ...
  async fetchSetFinishedData(): Promise<any> {
    if (this.options.ajax?.setFinishedUrl) {
      return this.options.ajax.setFinishedUrl();
    }
    throw new Error("setFinishedUrl is not defined");
  }

  // Function for calling contentUserDataUrl
  async fetchContentUserData(): Promise<any> {
    if (this.options.ajax?.contentUserDataUrl) {
      return this.options.ajax.contentUserDataUrl();
    }
    throw new Error("contentUserDataUrl is not defined");
  }
