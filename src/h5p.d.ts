...
export interface H5PIntegration {
  ...
  core?: {
    //required only when Embed Type = iframe
    scripts: string[];
    styles: string[];
  };
...
export interface H5PContent {
  ...
  contentUserData?: {
    [subContentId: string]: "RESET" | { state: string };
  }[] /* user preloaded data where RESET: will delete previous user state, and state: exp*/;
